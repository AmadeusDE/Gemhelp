...
graph TD
    ...
    ToolCall --> Man[man.go - Local ROFF Parser]
    ToolCall --> Tldr[tldr.go - Offline TLDR Client]
    ToolCall --> Wiki[wiki.go - Arch Wiki API Client]
```

## 2. File Roles & Descriptions

*   **`main.go`**: Entry point. Parses flags (`--no-cache`, `--tldr`, `--man`, `--wiki`) and manages subcommands (`man`, `tldr`, `wiki`). Detects executable base name (`os.Args[0]`) to route multi-call aliases (e.g. running the binary directly as `tldr` or `man`).
...
*   **`gemini.go`**: Orchestrates client creation with `google.golang.org/genai`. Formulates tools schema, triggers parallel tool executions in each turn, and applies exponential backoffs on `ResourceExhausted` errors, falling back through the `gemini-3.5-flash` -> `gemini-3.1-flash-lite` -> `gemini-2.5-flash` chain.
